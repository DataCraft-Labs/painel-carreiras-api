...
async def get_salary_distribution(
    position: str = "", location: str = "", company: str = "", db: AsyncSession = Depends(async_get_db)
):
    conditions = []
    if position:
    ...
    query = (
        select(Salary.base_salary.label("salary"), func.count().label("count"))
        .join(Posting, Posting.id == Salary.posting_id)
        .join(Position, Position.id == Posting.position_id)
    )

    if conditions:
        query = query.where(and_(*conditions))

    query = query.group_by(Salary.base_salary).order_by(Salary.base_salary)
    ...
    salary_distribution = {
        "salaries": [
            {"salary": format_salary_range(salary), "count": count} for salary, count in salary_counts.items()
        ],
        "statistics": {
...
async def get_top_used_skills(
    ...
    total_skills_query = (
        select(func.count(JobSkill.skill_id))
        .join(Posting, Posting.id == JobSkill.posting_id)
        .join(Position, Position.id == Posting.position_id)
    )
    ...
    query = (
        select(Skill.name.label("name"), func.count(JobSkill.skill_id).label("count"))
    ...
    query = query.group_by(Skill.name).order_by(func.count(JobSkill.skill_id).desc()).limit(5)
    ...
    return {"skills": [{"name": skill, "percentage": (count / total_skills) * 100} for skill, count in skills]}
